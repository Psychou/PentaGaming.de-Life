...
/* System Settings */
    ...
    noatm_timer = 22; //Time in minutes that players cannot deposit money after selling stolen gold.
    ...
    news_broadcast_cost = 2000; //Cost for a player to send a news station broadcast.
    ...
    escapeMenu_timer = 17; //Time required to pass before you can click the abort button in the escape menu.
    escapeMenu_displayExtras = true; //Display the players UID & serverName specified below in the escape menu.
    escapeMenu_displayText = "Besuche uns bald wieder!"; //Text displayed in the escape menu. Make it short.. around 20 characters.

    /* Fuel System Configurations */
    pump_service = false; //Allow users to use pump service on the map. Default = false
    fuel_cost = 70; //Cost of fuel per liter at fuel stations (if not defined for the vehicle already).
    ...
    gang_price = 175000; //Gang creation price. --Remember they are persistent so keep it reasonable to avoid millions of gangs.
    gang_upgradeBase = 22500; //The base cost for purchasing additional slots in a gang
    ...
    revive_fee = 8651; //Revive fee that players have to pay and medics only EMS(independent) are rewarded with this amount.
    ...
    paycheck_cop = 6491; //Payment for cops
    paycheck_civ = 3500; //Payment for civillians
    paycheck_med = 8531; //Payment for medics
    ...
    vehicle_rental_multiplier_COP = .50; //Cop Vehicle Rental Price = Config_Vehicle price * multiplier
    ...
    vehicle_sell_multiplier_MEDIC = .01; //Medic Vehicle Garage Sell Price = Vehicle Buy Price * multiplier
